...
def yielding_data(data: list[dict]):
    for id in data:
        yield id


def data_stream(data: list[dict]) -> None:
    log: dict = dict()
    events: dict = dict()
    event_type: str
    high_level: int = 0
    ...
    for key, value in events.items():
        print(f"{key.capitalize()} events: {value}")
    print()


def data_store(data: list[dict]) -> None:
    events: list[dict] = []
    event_type: str
    high_level: int = 0
    total_events: int = 0
    ...
    for log in data:
        events.append(log)
        event_type = log["event_type"]
        log[event_type] = log.get(event_type, 0) + 1
        if log["data"]["level"] >= 10:
            high_level += 1
        total_events += 1
